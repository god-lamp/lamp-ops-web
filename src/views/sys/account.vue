...
import { fetchAccounts } from '@/api/sys/account'
...
export default {
  ...
  methods: {
      ...
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          ...
          createArticle(this.temp).then(() => {
            this.accounts.unshift(this.temp)
      ...
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          ...
          updateArticle(tempData).then(() => {
            const index = this.accounts.findIndex(v => v.id === this.temp.id)
            this.accounts.splice(index, 1, this.temp)
            this.dialogFormVisible = false
            this.$notify({
              title: 'Success',
    ...
    formatJson(filterVal) {
      return this.accounts.map(v => filterVal.map(j => {
        if (j === 'timestamp') {
